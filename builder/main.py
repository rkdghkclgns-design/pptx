...
def main() -> None:
    ...
    try:
        ...
        # Note: NotebookLM URLs are private (login required).
        # Content must come from text_source (user paste) or file uploads.
        if notebook_url:
            print(f"NotebookLM URL recorded: {notebook_url} (content should be in text_source)")

        content = "\n\n".join(content_parts)
        if not content.strip():
            raise RuntimeError(
                "소스 데이터가 없습니다. NotebookLM에서 내용을 복사하여 "
                "'텍스트 직접 입력'에 붙여넣거나, 파일을 업로드하세요."
            )
